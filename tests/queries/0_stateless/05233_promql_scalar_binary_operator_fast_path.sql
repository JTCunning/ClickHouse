...
-- (e.g. `sum(a) / sum(b)`): such shapes use a CROSS JOIN instead of keyed vector matching.
...
-- (while still known to the converter, unlike a completely nonexistent metric).
...
SELECT '-- r09-like: fused sum-max then outer minus vs 0';
SELECT * FROM prometheusQuery('prometheus', 'sum(m1) - (sum(m2) - max(m2)) > 0', 130) ORDER BY tags;

SELECT '-- r09-like plan uses a cross join, not HashJoin';
SELECT countIf(explain LIKE '%Type: cross%') > 0
FROM (EXPLAIN PLAN SELECT * FROM prometheusQuery('prometheus', 'sum(m1) - (sum(m2) - max(m2)) > 0', 130));
SELECT countIf(explain LIKE '%HashJoin%')
FROM (EXPLAIN SELECT * FROM prometheusQuery('prometheus', 'sum(m1) - (sum(m2) - max(m2)) > 0', 130));

SELECT '-- labeled matching control still uses HashJoin';
SELECT countIf(explain LIKE '%HashJoin%') > 0
FROM (EXPLAIN SELECT * FROM prometheusQuery('prometheus', 'sum by (host) (m1) / sum by (host) (m2)', 130));
