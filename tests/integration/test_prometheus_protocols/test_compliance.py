...
# Metric used only by test_compliance_ingest_prometheus_openmetrics_text_export (inserted via PRW in that test).
# Name/labels/timestamps follow the same conventions as generate_compliance_data.py (demo_*, demo.promlabs.com, 15s steps).
...
def test_compliance_ingest_prometheus_openmetrics_text_export():
    ...
    (snappy-compressed PRW to ClickHouse HTTP /write), not via the OpenMetrics file ingest path.
    """
    # Counter-style samples: monotonic increase on a realistic 15s scrape interval (see DATA_INTERVAL in generate_compliance_data).
    samples = {
    ...
    send_protobuf_to_remote_write(
        ...
        convert_time_series_to_protobuf(
            [
                (
                    {
                        # Label names must be sorted lexicographically for TimeSeries remote write.
