...
def assert_query_log_has_finish_with_written_rows(
    ...
    """
    Assert the remote write produced a correlated QueryFinish row with written
    rows/bytes, marked as an internal secondary query of the original request.
    """
    node.query("SYSTEM FLUSH LOGS query_log")
    assert_eq_with_retry(
        node,
...
def test_remote_write_appears_in_query_log_with_written_rows():
    ...
    request via child query ids "<X-ClickHouse-Query-Id>:<target table kind>" and
    are marked as internal secondary queries keeping the request's query id as
    initial_query_id.
    """
    query_id = f"prometheus-query-log-test-{uuid.uuid4()}"
