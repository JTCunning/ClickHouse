...
Frame generation runs in parallel worker processes (`--workers`, default half the CPUs); POSTs are serial. As a reference point, scale=16 (~15.7 billion samples) loaded at ~2.4 million samples/sec on an 8-vCPU machine — about 1¾ hours; scale=1 takes proportionally less. `--dry-run` prints corpus stats without loading.
