    ...
    , WithContext(local_context->getGlobalContext())
{
    ...
    if (!(*settings)[TimeSeriesSetting::store_min_time_and_max_time]
        && (*settings)[TimeSeriesSetting::insert_cache_max_size_bytes]
        && isInnerTable(ViewTarget::Tags))
    {
...
{
    const bool changes_id_generator = std::ranges::any_of(params, [](const AlterCommand & command)
    {
        if (command.type == AlterCommand::MODIFY_SETTING)
            return command.settings_changes.tryGet("id_generator") != nullptr;
        if (command.type == AlterCommand::RESET_SETTING)
            return std::ranges::find(command.settings_resets, "id_generator") != command.settings_resets.end();
        return false;
    });

    if (changes_id_generator && insert_cache)
        insert_cache->clear();
...
| `insert_cache_max_size_bytes` | UInt64 | 16777216 | Maximum memory used by the replica-local insert cache when `store_min_time_and_max_time` is false. Set to 0 to disable the cache |
