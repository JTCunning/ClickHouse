...
```sql
CREATE TABLE my_table ENGINE=TimeSeries
SAMPLES INNER COLUMNS (samples SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp UInt32 CODEC(Delta, T64, ZSTD(3)), value Float32 CODEC(ALP, ZSTD(3))))))
```
...
Columns the engine creates itself get time-series compression codecs on the `timestamp` element of `samples`,
`bucket`, `min_time` and `max_time`: `CODEC(Delta, T64, ZSTD(3))`. The `value` element of `samples` gets
`CODEC(ALP, ZSTD(3))`. Near-monotonic timestamps barely compress under generic codecs and can otherwise
dominate the on-disk size of the samples table. The `samples` column uses
[tuple element codecs](/reference/statements/create/table/codec#tuple-element-codecs); these declarations
are allowed while the `enable_time_series_table` setting is enabled, the `enable_tuple_element_codecs`
setting is not required. The engine enables `ALP` for its inner samples and recent samples tables without
requiring `enable_alp_codec` to be set. The codecs are generated for MergeTree-family inner engines only,
other engines ignore codecs.
...
The generated timestamp codecs are `CODEC(Delta, T64, ZSTD(3))`,
and the generated value codec is `CODEC(ALP, ZSTD(3))`.
...
ENGINE = TimeSeries
SETTINGS version = 6, recent_samples_ttl_seconds = 345600, samples_bucket_step_seconds = 3600, recent_samples_bucket_step_seconds = 900
SAMPLES INNER COLUMNS
(
    `id` Tuple(UInt64, LowCardinality(UUID)),
    `samples` SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp DateTime64(3) CODEC(Delta, T64, ZSTD(3)), value Float64 CODEC(ALP, ZSTD(3))))),
    `bucket` DateTime64(3) CODEC(Delta, T64, ZSTD(3)),
    `min_time` SimpleAggregateFunction(min, DateTime64(3)) CODEC(Delta, T64, ZSTD(3)),
...
```sql
CREATE TABLE my_table ENGINE=TimeSeries
SAMPLES INNER COLUMNS (samples SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp DateTime64(6) CODEC(Delta, T64, ZSTD(3)), value Float32 CODEC(ALP, ZSTD(3))))))
```
...
| 6 | The [samples](#samples-table) and [recent samples](#recent-samples-table) tables store buckets of samples (columns `samples`, `bucket`, `min_time`, `max_time`) instead of one sample per row (columns `timestamp`, `value`) |
