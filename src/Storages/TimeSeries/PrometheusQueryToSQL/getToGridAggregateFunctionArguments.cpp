...
ASTs getToGridAggregateFunctionArguments(const SQLQueryPiece & range_vector, ConverterContext & context)
{
    ...
    switch (range_vector.store_method)
    {
        ...
        {
            /// The selector returns one Array(Tuple(timestamp, value)) per bucket as `time_series`.
            /// timestamps: time_series.1
            /// values:     time_series.2
            ASTPtr samples = make_intrusive<ASTIdentifier>(ColumnNames::TimeSeries);
            timestamps = makeASTFunction("tupleElement", samples, make_intrusive<ASTLiteral>(1));
            values = makeASTFunction(
