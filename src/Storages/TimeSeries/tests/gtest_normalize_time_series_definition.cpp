...
namespace
{
    ...
    const String default_timestamp_codec = "CODEC(Delta, T64, ZSTD(3))";
    const String default_value_codec = "CODEC(ALP, ZSTD(3))";

    String makeBucketedSamplesColumns(const String & id_type, const String & timestamp_type, const String & value_type, bool with_generated_codecs = true)
    {
        String timestamp_codec;
        String value_codec;
        String column_codec;
        if (with_generated_codecs)
        {
            timestamp_codec = " " + default_timestamp_codec;
            value_codec = " " + default_value_codec;
            column_codec = " " + default_timestamp_codec;
        }
        return "`id` " + id_type + ", `samples` SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp " + timestamp_type
            + timestamp_codec + ", value " + value_type + value_codec + "))), `bucket` " + timestamp_type + column_codec
            + ", `min_time` SimpleAggregateFunction(min, " + timestamp_type + ")" + column_codec
            + ", `max_time` SimpleAggregateFunction(max, " + timestamp_type + ")" + column_codec;
    }
}
...
TEST_F(NormalizeTimeSeriesDefinitionTest, DefaultDefinition)
{
    ...
    String samples_columns = makeBucketedSamplesColumns(default_id_type, "DateTime64(3)", "Float64");
    ...
    EXPECT_EQ(extractInnerEngine(definition, "SAMPLES"),
        "AggregatingMergeTree PARTITION BY toYYYYMM(bucket) ORDER BY (id, bucket) SETTINGS index_granularity = 256, index_granularity_bytes = 524288");
    EXPECT_EQ(extractInnerEngine(definition, "RECENT SAMPLES"),
        "AggregatingMergeTree PARTITION BY toStartOfInterval(bucket, toIntervalHour(5)) ORDER BY (id, bucket) "
        "TTL bucket + toIntervalSecond(346500) SETTINGS index_granularity = 256, index_granularity_bytes = 131072, ttl_only_drop_parts = 1");
...
TEST_F(NormalizeTimeSeriesDefinitionTest, BucketedSamplesTupleCodecs)
{
    /// Generated tuple-element codecs use the current timestamp and value codecs. A column the user
    /// declared keeps the user's codecs, including no codec. A Memory inner table gets no codecs.

    auto definition = normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries");
    EXPECT_EQ(extractInnerColumns(definition, "SAMPLES"), makeBucketedSamplesColumns(default_id_type, "DateTime64(3)", "Float64"));
    ...
    EXPECT_EQ(
        extractInnerColumns(definition, "SAMPLES"),
        "`id` UUID, `samples` SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp DateTime64(3), value Float64))), "
        "`bucket` DateTime64(3) CODEC(Delta, T64, ZSTD(3)), `min_time` SimpleAggregateFunction(min, DateTime64(3)) CODEC(Delta, T64, ZSTD(3)), "
        "`max_time` SimpleAggregateFunction(max, DateTime64(3)) CODEC(Delta, T64, ZSTD(3))");

    Settings query_settings;
    query_settings[Setting::default_table_engine] = DefaultTableEngine::MergeTree;
    definition = normalizeNewTableWithSettings(
        "CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS recent_samples_ttl_seconds = 0 SAMPLES INNER ENGINE = Memory",
        query_settings);
    EXPECT_EQ(extractInnerColumns(definition, "SAMPLES"), makeBucketedSamplesColumns(default_id_type, "DateTime64(3)", "Float64", /*with_generated_codecs=*/ false));
    ...
    EXPECT_TRUE(extractInnerColumns(definition, "SAMPLES").contains("timestamp DateTime64(3) CODEC(Delta, LZ4), value Float64 CODEC(LZ4)")) << definition;
    EXPECT_TRUE(extractInnerColumns(definition, "SAMPLES").contains("`bucket` DateTime64(3) CODEC(Delta, T64, ZSTD(3))")) << definition;
}
...
TEST_F(NormalizeTimeSeriesDefinitionTest, DefaultTableEngineChoosesInnerEngineFamily)
{
    ...
    EXPECT_TRUE(extractInnerEngine(definition, "SAMPLES").starts_with("ReplicatedAggregatingMergeTree ")) << definition;
    EXPECT_TRUE(extractInnerEngine(definition, "RECENT SAMPLES").starts_with("ReplicatedAggregatingMergeTree ")) << definition;
    ...
    EXPECT_TRUE(extractInnerEngine(definition, "SAMPLES").starts_with("SharedAggregatingMergeTree ")) << definition;
    EXPECT_TRUE(extractInnerEngine(definition, "RECENT SAMPLES").starts_with("SharedAggregatingMergeTree ")) << definition;
...
TEST_F(NormalizeTimeSeriesDefinitionTest, TypesDeclaredInInnerColumns)
{
    auto definition = normalizeNewTable(
        "CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 5 SAMPLES INNER COLUMNS (timestamp DateTime64(6), value Float32) TAGS INNER COLUMNS (id UInt64)");
...
{
    auto definition = normalizeNewTable("CREATE TABLE db.ts ENGINE = TimeSeries SETTINGS version = 5, id_type = 'UInt64'");
...
TEST_F(NormalizeTimeSeriesDefinitionTest, OuterColumns)
{
    ...
    EXPECT_EQ(extractInnerColumns(definition, "SAMPLES"),
        makeBucketedSamplesColumns(default_id_type, "UInt32", "Float32"));
...
TEST_F(NormalizeTimeSeriesDefinitionTest, DeclaredEnginesWithoutKeysGetGeneratedKeys)
{
    ...
    EXPECT_EQ(extractInnerEngine(definition, "SAMPLES"),
        "MergeTree PARTITION BY toYYYYMM(bucket) ORDER BY (id, bucket) SETTINGS index_granularity = 256, index_granularity_bytes = 524288");
    EXPECT_EQ(extractInnerEngine(definition, "RECENT SAMPLES"),
        "MergeTree PARTITION BY toStartOfInterval(bucket, toIntervalHour(5)) ORDER BY (id, bucket) "
        "TTL bucket + toIntervalSecond(346500) SETTINGS index_granularity = 256, index_granularity_bytes = 131072, ttl_only_drop_parts = 1");
    EXPECT_EQ(extractInnerEngine(definition, "TAGS"),
        "AggregatingMergeTree PRIMARY KEY metric_name ORDER BY (metric_name, id) SETTINGS index_granularity = 8192, allow_dimensions_outside_sorting_key = 1");
    EXPECT_EQ(extractInnerEngine(definition, "METRIC FAMILIES"), "ReplacingMergeTree ORDER BY metric_family_name");
...
TEST_F(NormalizeTimeSeriesDefinitionTest, CreateAsCopiesInnerDefinitions)
{
    ...
    const String src = normalizeNewTable(
        "CREATE TABLE db.src ENGINE = TimeSeries SETTINGS version = 5 SAMPLES INNER COLUMNS (timestamp DateTime64(6) CODEC(Delta, ZSTD(1)), extra UInt8) "
    ...
    definition = normalizeNewTableAs("CREATE TABLE db.copy AS db.src ENGINE = TimeSeries SETTINGS version = 5, aggregate_min_time_and_max_time = 0", src);
    EXPECT_EQ(extractInnerColumns(definition, "SAMPLES"),
    ...
    definition = normalizeNewTableAs("CREATE TABLE db.copy AS db.src ENGINE = TimeSeries SETTINGS version = 5 SAMPLES INNER COLUMNS (value Float32)", src);
    ...
    definition = normalizeNewTableAs("CREATE TABLE db.copy AS db.src ENGINE = TimeSeries SETTINGS version = 5 SAMPLES db.ext_data",
        normalizeNewTable("CREATE TABLE db.src ENGINE = TimeSeries SETTINGS version = 5 SAMPLES INNER COLUMNS (timestamp DateTime64(3) CODEC(ZSTD(5)))"), params);
    EXPECT_FALSE(definition.contains("ext_data SAMPLES INNER COLUMNS")) << definition;
    EXPECT_EQ(extractInnerColumns(definition, "RECENT SAMPLES"), "`id` UInt64, `timestamp` DateTime64(6) CODEC(Delta, T64, ZSTD(3)), `value` Float64 CODEC(ALP, ZSTD(3))");
}
...
TEST_F(NormalizeTimeSeriesDefinitionTest, CreateAsTableWithExternalTargetTables)
{
    ...
    EXPECT_EQ(extractInnerColumns(definition, "SAMPLES"),
        makeBucketedSamplesColumns("UInt64", "DateTime64(6)", "Float64") + ", `extra` UInt8");
    EXPECT_EQ(extractInnerColumns(definition, "TAGS"),
