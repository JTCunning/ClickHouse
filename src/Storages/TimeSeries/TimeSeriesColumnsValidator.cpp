...
void TimeSeriesColumnsValidator::validateTargetColumnsImpl(ViewTarget::Kind target_kind, const ColumnsDescription & target_columns) const
{
    ...
    switch (target_kind)
    {
        ...
        {
            /// External (or manually maintained) data tables created before `metric_locality_id` may omit it;
            /// `timeSeriesSelector` and remote-write paths handle absence (synthesize or skip the column).
            if (const auto * locality_column = target_columns.tryGet(TimeSeriesColumnNames::MetricLocalityId))
                validateColumnForMetricLocalityId(*locality_column);
...
void TimeSeriesColumnsValidator::validateColumnForMetricLocalityId(const ColumnDescription & column) const
{
    ...
    if (!which.isUInt())
    {
        throw Exception(ErrorCodes::INCOMPATIBLE_COLUMNS, "Column {} has illegal data type {}, expected UInt*",
