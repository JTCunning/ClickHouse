...
namespace
{
    ...
    /// Whether the samples tables of the specified version store buckets of samples
    /// (versions before 6 store one sample per row).
    bool hasBucketedSamples(UInt64 version)
    {
        return version >= TimeSeriesVersion::MIN_WITH_BUCKETED_SAMPLES;
    }
    ...
    std::optional<std::pair<DataTypePtr, DataTypePtr>> trySplitSamplesDataType(const DataTypePtr & type)
    {
    ...
    /// Makes the type of the `samples` column of the samples table:
    /// `SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(timestamp <timestamp_type>, value <value_type>)))`.
    DataTypePtr makeSamplesDataType(const DataTypePtr & timestamp_type, const DataTypePtr & value_type)
    {
    ...
    /// Makes `CODEC(Delta, T64, ZSTD(3))`: the codec of the generated columns holding timestamps (the `timestamp` element
    /// of `samples`, `bucket`, `min_time` and `max_time`).
    ASTPtr makeTimestampCodecAST()
    {
    ...
    /// Makes `CODEC(ALP, ZSTD(3))`: the codec of the generated `value` element of `samples`.
    ASTPtr makeValueCodecAST()
    {
        return makeASTFunction("CODEC", make_intrusive<ASTIdentifier>("ALP"), makeASTFunction("ZSTD", make_intrusive<ASTLiteral>(UInt64{3})));
    }

    /// Makes the declaration of the type of the generated `samples` column (see `makeSamplesDataType`) with the codecs of
    /// the tuple elements: `Array(Tuple(timestamp <timestamp_type> CODEC(Delta, T64, ZSTD(3)), value <value_type> CODEC(ALP, ZSTD(3))))`.
    ASTPtr makeSamplesDataTypeASTWithCodecs(const DataTypePtr & timestamp_type, const DataTypePtr & value_type)
    {
        auto type_ast = dataTypeToAST(makeSamplesDataType(timestamp_type, value_type));

        /// `SimpleAggregateFunction(timeSeriesGroupArray, Array(Tuple(...)))`: the tuple is the argument of the array,
        /// and the array is the second argument of the aggregate function type.
        const auto & array_ast = type_ast->getArguments()->children.at(1)->as<ASTDataType &>();
        auto & tuple_ast = array_ast.getArguments()->children.at(0)->as<ASTTupleDataType &>();

        auto make_set_codec_operation = [](size_t element_index, ASTPtr codec) -> ASTPtr
        {
        ...
        ASTs codec_operations;
        codec_operations.push_back(make_set_codec_operation(0, makeTimestampCodecAST()));
        codec_operations.push_back(make_set_codec_operation(1, makeValueCodecAST()));
        tuple_ast.setCodecOperations(std::move(codec_operations));
        return type_ast;
    }

    /// Whether a type declaration has codecs of tuple elements at any nesting level.
    bool hasTupleElementCodecs(const IAST & type_ast)
    {
        if (const auto * tuple_ast = type_ast.as<ASTTupleDataType>(); tuple_ast && tuple_ast->getCodecOperations())
            return true;
        return std::ranges::any_of(type_ast.children, [](const ASTPtr & child) { return hasTupleElementCodecs(*child); });
    }

    /// Returns `bucket` for `DateTime` and `DateTime64` timestamps, or `toDateTime(bucket)` for raw `UInt32` timestamps.
    /// The generated partition keys (e.g. `toYYYYMM(bucket)`) and TTL (`bucket + toIntervalSecond(...)`) of the samples tables
    /// need a date-time expression.
        ...
        const StorageID & table_id)
    {
        ...
        for (const auto & column : inner_columns->columns->children)
        {
            ...
            {
                auto types = trySplitSamplesDataType(column_type);
                if (!types)
                    throw Exception(ErrorCodes::BAD_TYPE_OF_FIELD,
        ...
        const StorageID & table_id)
    {
        for (const auto & column : external_columns)
        {
            ...
            {
                if (auto types = trySplitSamplesDataType(column.type))
                {
    ...
    bool isGeneratedInnerColumn(const ASTColumnDeclaration & column, ViewTarget::Kind inner_table_kind, const TimeSeriesSettings & settings)
    {
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                auto is_generated_timestamp_codec = [](const ASTPtr & column_codec)
                {
                    if (!column_codec)
                        return true;
                    auto codec_name = column_codec->formatWithSecretsOneLine();
                    return (codec_name == "CODEC(DoubleDelta, ZSTD(1))") || (codec_name == "CODEC(Delta, T64, ZSTD(3))");
                };

                if (hasBucketedSamples(settings))
                {
                    /// The generated "samples" column has codecs of the tuple elements (see `normalizeInnerColumns`).
                    /// A column without codecs or with the column-level codec `ZSTD(3)` counts too, in case it was
                    /// generated by another version of the server.
                    if (name == TimeSeriesColumnNames::Samples)
                    {
                        auto types = trySplitSamplesDataType(type);
                        if (!types || !is_timestamp_type(*types->first) || !is_scalar_type(*types->second))
                            return false;
                        if (type_name != makeSamplesDataType(types->first, types->second)->getName())
                            return false;
                        if (hasTupleElementCodecs(*column.getType()))
                            return !codec;
                        return !codec || (codec->formatWithSecretsOneLine() == "CODEC(ZSTD(3))");
                    }

                    if (!is_generated_timestamp_codec(codec))
                        return false;
                ...
                if (name == TimeSeriesColumnNames::Timestamp)
                {
                    if (!is_timestamp_type(*type))
                        return false;
                    return !codec || is_version_0 || is_generated_timestamp_codec(codec);
                }
    ...
    void removeGeneratedInnerEngine(ASTStorage & inner_engine, ViewTarget::Kind inner_table_kind, const TimeSeriesSettings & settings)
    {
        ...
        auto partitioning_is_one_of = [&](const Strings & expressions)
        {
        ...
        auto ttl_is_one_of = [&](const Strings & expressions)
        {
        ...
        switch (inner_table_kind)
        {
            case ViewTarget::Samples:
            {
                const bool has_bucketed_samples = hasBucketedSamples(settings);
                if (engine_name != (has_bucketed_samples ? "AggregatingMergeTree" : "MergeTree"))
                    return;
                if (sorting_key_equals(has_bucketed_samples ? "id, bucket" : "id, timestamp"))
                ...
                else if (has_bucketed_samples)
                    expected_partition_by = {"toYYYYMM(bucket)", "toYYYYMM(toDateTime(bucket))"};
                if (partitioning_is_one_of(expected_partition_by))
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                if (partition_by)
                    ...
                    expected_partition_by = {"toStartOfInterval(bucket, toIntervalHour(5))", "toStartOfInterval(toDateTime(bucket), toIntervalHour(5))"};
                else
                ...
                UInt64 bucketed_ttl_seconds = ttl_seconds + settings[TimeSeriesSetting::recent_samples_bucket_step_seconds].value;
                Strings expected_ttl = has_bucketed_samples
                    ? Strings{fmt::format("bucket + toIntervalSecond({})", bucketed_ttl_seconds), fmt::format("toDateTime(bucket) + toIntervalSecond({})", bucketed_ttl_seconds)}
                    : Strings{fmt::format("toDateTime(timestamp) + toIntervalSecond({})", ttl_seconds)};
                if (ttl_is_one_of(expected_ttl))
        ...
        const Settings & query_settings)
    {
        ...
        /// Throws if the engine is not a MergeTree-family engine; `what` names the feature which needs one.
        auto require_merge_tree = [&](std::string_view what)
        {
            if (!is_merge_tree())
                throw Exception(ErrorCodes::INVALID_SETTING_VALUE,
                    "{}: {} requires a MergeTree-family engine of the inner {} table", table_id.getNameForLogs(), what, inner_table_kind);
        };
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                /// The rows of the same series and bucket are merged by `AggregatingMergeTree`;
                /// the tables of versions before 6 use `MergeTree` with one sample per row.
                const bool has_bucketed_samples = hasBucketedSamples(settings);
                if (!inner_engine.engine)
                    set_engine(has_bucketed_samples ? "AggregatingMergeTree" : "MergeTree");

                if (needs_sorting_key())
                {
                    set_sorting_key({make_intrusive<ASTIdentifier>(TimeSeriesColumnNames::ID),
                        make_intrusive<ASTIdentifier>(has_bucketed_samples ? TimeSeriesColumnNames::Bucket : TimeSeriesColumnNames::Timestamp)});
                }
                ...
                if (inner_table_kind == ViewTarget::Samples)
                {
                    ...
                    {
                        require_merge_tree("Setting `samples_partition_by`");
                        set_partition_by(partition_by->clone());
                    }
                    else if (has_bucketed_samples && is_merge_tree() && !has_partition_by())
                    {
                ...
                require_merge_tree("The TTL defined by the `recent_samples_ttl_seconds` setting");
        ...
        const StorageID & table_id)
    {
        ...
        switch (target_kind)
        {
            ...
            case ViewTarget::RecentSamples:
            {
                ...
                if (target_table_columns.tryGet(String(TimeSeriesColumnNames::Timestamp))
                    || target_table_columns.tryGet(String(TimeSeriesColumnNames::Value)))
                {
                    throw Exception(ErrorCodes::INCORRECT_QUERY,
                        "{}: Columns {} and {} must not exist in the {} table of a bucketed TimeSeries table, please recreate the table",
                        table_id.getNameForLogs(), TimeSeriesColumnNames::Timestamp, TimeSeriesColumnNames::Value, target_kind);
                }

                check_column(TimeSeriesColumnNames::Samples);
                const auto * samples_col = target_table_columns.tryGet(String(TimeSeriesColumnNames::Samples));
                auto split_samples = trySplitSamplesDataType(samples_col->type);
                if (!split_samples
                    || !split_samples->first->equals(*resolved_types.timestamp_type)
                    || !split_samples->second->equals(*resolved_types.scalar_type))
                {
                    throw Exception(
                        ErrorCodes::BAD_TYPE_OF_FIELD,
