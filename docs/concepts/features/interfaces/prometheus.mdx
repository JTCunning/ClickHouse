...
For HTTP API requests, enable `enable_time_series_table` in the profile of the API user. To accept Remote Write 2.0, also enable `enable_prometheus_remote_write_v2` in that profile.
...
Remote Write 1.0 is enabled by default. To accept Remote Write 2.0 (`io.prometheus.write.v2.Request`), enable `enable_prometheus_remote_write_v2` for the API user:

```sql
SET enable_prometheus_remote_write_v2 = 1;
```

Remote Write 2.0 stores float samples, metric metadata, and native histograms.
Native histograms on Remote Write 2.0 use the same storage as Remote Write 1.0: a current-version `TimeSeries` table stores them, and an older table drops them and still accepts the request.
ClickHouse rejects the complete request when exemplars are present.
ClickHouse rejects the complete request when a sample has a nonzero `start_timestamp`.
Senders can set `send_exemplars: false` to omit exemplars.
...
:::note
Prometheus 3.10 and later send native histograms with custom buckets over Remote Write 2.0. Remote Write 1.0 still drops them on the sending side with a warning. Earlier Prometheus versions send them over Remote Write 1.0 without bucket bounds, which makes ClickHouse reject the request. Other senders, for example the OpenTelemetry Collector with `convert_explicit_histograms_to_nhcb` enabled, send custom-bucket histograms over Remote Write 1.0, and ClickHouse stores them.
:::
