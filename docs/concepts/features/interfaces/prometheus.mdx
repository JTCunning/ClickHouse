...
Remote Write 2.0 support is float-only. ClickHouse stores float samples and metric metadata.
ClickHouse rejects the complete request when native histograms are present.
...
Prometheus `send_native_histograms` is a no-op for `io.prometheus.write.v2.Request`, so a sender cannot use it to omit native histograms.
A sender drops native histogram series with a `write_relabel_configs` rule on the `__type__` label.
Prometheus sets `__type__` only when it runs with `--enable-feature=type-and-unit-labels`.
Without that feature flag the rule matches nothing, and ClickHouse rejects every request that carries a native histogram.
The rule removes the complete series, not only its histogram samples.
  ...
  - url: https://clickhouse.example.com:8443/prometheus/api/v1/write?database=prometheus&table=metrics
    ...
    write_relabel_configs:
      - source_labels: [__type__]
        regex: histogram|gaugehistogram
        action: drop
```
