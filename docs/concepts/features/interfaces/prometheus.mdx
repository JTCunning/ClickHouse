...
The `query` handler evaluates PromQL through the Prometheus HTTP query API. Configure handlers for the instant and range query endpoints:
...
| Name | Default | Description |
|---|---|---|
| `url` / `headers` / `method` | none | Filters used to find a matching handler. Similar to the fields with the same names in [`<http_handlers>`](/concepts/features/interfaces/http). |
| `table` | none | The name of the [`TimeSeries`](/reference/engines/table-engines/integrations/time-series) table used to evaluate PromQL. The name can include a database. |
| `database` | none | The database containing the table when `table` does not include a database. |
