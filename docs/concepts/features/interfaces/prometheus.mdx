---
description: 'Use ClickHouse to store Prometheus metrics, ingest and read Prometheus protocols, and evaluate PromQL.'
...
ClickHouse can store Prometheus metrics in a [`TimeSeries`](/reference/engines/table-engines/integrations/time-series) table and serve the Prometheus HTTP API. The same API handler supports remote write, remote read, instant PromQL queries, and range PromQL queries.

ClickHouse can also expose its own server metrics for a Prometheus server to scrape. This is a separate use case from storing and querying metrics in a `TimeSeries` table.
