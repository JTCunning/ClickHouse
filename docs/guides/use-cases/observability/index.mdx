...
ClickHouse can store Prometheus metrics in a [`TimeSeries`](/reference/engines/table-engines/integrations/time-series) table and evaluate PromQL queries. See the [PromQL support matrix](/guides/use-cases/observability/promql-support-matrix) for function-level support and compliance results.
