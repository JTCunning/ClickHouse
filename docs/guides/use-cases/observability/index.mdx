...
ClickHouse can store Prometheus metrics in a [`TimeSeries`](/reference/engines/table-engines/integrations/time-series) table and evaluate PromQL queries. See [Prometheus protocols and PromQL](/interfaces/prometheus) for ingestion, query, and visualization configuration.
